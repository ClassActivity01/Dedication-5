<%@ Page Title="Maintain Component" Language="C#" MasterPageFile="~/navigation.Master" AutoEventWireup="true" CodeBehind="3-13.aspx.cs" Inherits="Test._3_13" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="maincontent" runat="server">
<h1 class="default-form-header">Maintain Component <span id="item_ID"></span></h1>
...
<div class="searchDiv">
	<form id="search_form">
		<fieldset>
		<legend>Search for Component: </legend>
			<div class="row">
			<div class="col-sm-4">
				<label for="search_criteria" class="control-label">Search Criteria: </label>
				<input type="text" class="form-control" id="search_criteria" placeholder="Search Criteria" title ="Enter the components details to search by" data-toggle ="tooltip" data-placement ="bottom">
			</div>
			<div class="col-sm-4">
				<label for="search_category">Search By: </label>
				<select class="form-control" id="search_category" title ="Select what to search the component by" data-toggle ="tooltip" data-placement="bottom">
                    <option value="All">All</option>
					<option value="ID">Component No.</option>
			...
			<div class="row">
				...
				<div class="col-sm-4">
					<button id="submitSearch" type ="submit" class ="searchButton" title ="Click here to search for the component" data-toggle ="tooltip" data-placement ="left"><i class="fa fa-search" aria-hidden="true"></i> Search</button>
				</div>
...
<div class="modal fade" id="ResultModal">
	...
	<div class="modal-content">
		...
		<h4 class="modal-title">Select a Component</h4>
		</div>
		<div class="modal-body">
			<select multiple class="form-control" id="component_search_result" title ="Select the component">
			</select>
		</div>
		...
		<button type="button" class="btn btn-secondary modalbutton" id="loadComponentDetails" title ="Click to load the components details">Load Component Details</button>
		</div>
	</div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->

<style>    #div_3_13 { display: none;
    }
</style>
...
<div id="div_3_13">
	...
	<fieldset>
        ...
        <div class="col-sm-4">
            <button onclick="return deleteComponent()" class = "form-custom-button-columns" title ="Click to remove the component" data-toggle ="tooltip" data-placement ="left"><i class="fa fa-times" aria-hidden="true"></i> Remove Component</button>
		</div>	
		<div class="col-sm-4">
            <button onclick="return updateComponent()" class = "form-custom-button-columns" title ="Click to update the component details" data-toggle ="tooltip"><i class="fa fa-wrench" aria-hidden="true"></i> Update Component</button>
		</div>
	</div>
...
<div class="modal fade" id="supplierSearchModal">
	...
	<div class="modal-content">
		...
		<button type="button" class="btn btn-secondary modalbutton" onclick="addSupplier()" title ="Click to assign the supplier to the component">Assign Supplier to Raw Material</button>
		</div>
	</div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
function updateComponent()
{
    ...
    if (warnings == "") {
        $.ajax({
            type: "PUT",
            url: "api/Component/" + component_ID,
...
function deleteComponent() {
    ...
    if (component_ID == "" || component_ID == null) {
        var warnings = "Cannot Delete - No Component has been chosen. <br/>";
        $("#w_info").html(warnings);
        return false;
    }

    alertify.confirm('Removal Confirmation', 'Are you sure you want to delete this Component?', function () {
        $.ajax({
            type: "DELETE",
            url: "api/Component/" + component_ID,
            contentType: "application/json; charset=utf-8",
...
$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if (result[0] == "true") {
                    ...
                    searched_components = JSON.parse(result[1]).components;

                    $('#component_search_result').empty();
                    if (searched_components.length == 0) {
                        $("#component_search_result").append("<option value=''>No results found.</option>");
                    }
                    else {
                        for (var k = 0; k < searched_components.length; k++) {
                            var html = '<option value="' + searched_components[k].Component_ID + '">' + searched_components[k].Component_ID + ' - ' + searched_components[k].Name + '</option>';

                            $("#component_search_result").append(html);
                        }
                    }
                }
    ...
    $('#loadComponentDetails').on('click', function () {

        component_ID = $('#component_search_result').val();

        if (component_ID == "" || component_ID == null)
            alertify.alert('Error', 'No Component has been chosen!');
        else
            $.ajax({
                type: "GET",
                url: "api/Component/" + component_ID,
                ...
                success: function (msg) {
                    ...
                    if (result[0] == "true") {
                        ...
                        var component_details = JSON.parse(result[1]).components[0];

                        $("#div_3_13").show();

                        $("#item_ID").html("#" + component_ID + " ");

                        $("#comp_name").val(component_details.Name);
                        $("#comp_desc").val(component_details.Description);
                        $("#comp_dim").val(component_details.Dimension);
                        $("#comp_price").val(component_details.Unit_Price);
                        $("#comp_quantity").val(component_details.Quantity);
                        $("#comp_min").val(component_details.Min_Stock);

                        $('#ResultModal').modal('hide');
                        suppliers = [];

                        for (var a = 0; a < component_details.Suppliers.length; a++) {
                            var ID = component_details.Suppliers[a].Supplier_ID
                            ...
                            $.ajax({
                                ...
                                success: function (msg) {
                                    ...
                                    if (result[0] == "true") {
                                        var supp = JSON.parse(result[1]).suppliers[0];

                                        $("#supplier_name_" + ID).val(supp.Name);
                                        $("#price_" + ID).val(component_details.Suppliers[a].unit_price);

                                        if (component_details.Suppliers[a].is_preferred == true)
