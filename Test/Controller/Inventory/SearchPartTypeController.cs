...
{
    public class SearchPartTypeController : ApiController
    {
                        ...
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                where p.Name == criteria || p.Abbreviation == criteria 
                                    || p.Description == criteria || p.Dimension == criteria
                                select new
                                {
                        ...
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                where p.Name.Contains(criteria) || p.Abbreviation.Contains(criteria)
                                    || p.Description.Contains(criteria) || p.Dimension.Contains(criteria)
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "Abb")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                from p in db.Part_Type
                                orderby p.Name
                                where p.Abbreviation.Contains(criteria)
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
        ...
        public string Put(int id, HttpRequestMessage value)
        {
                ...
                if (method == "Exact")
                {
                    if (category == "All")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                from p in db.Part_Type
                                join s in db.Part_Supplier on p.Part_Type_ID equals s.Part_Type_ID
                                orderby p.Name
                                where (p.Name == criteria || p.Abbreviation == criteria
                                    || p.Description == criteria || p.Dimension == criteria) && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "Abb")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                join s in db.Part_Supplier on p.Part_Type_ID equals s.Part_Type_ID
                                orderby p.Name
                                where p.Abbreviation == criteria && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "Dimension")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                join s in db.Part_Supplier on p.Part_Type_ID equals s.Part_Type_ID
                                orderby p.Name
                                where p.Dimension == criteria && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "All")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                orderby p.Name
                                where (p.Name.Contains(criteria) || p.Abbreviation.Contains(criteria)
                                    || p.Description.Contains(criteria) || p.Dimension.Contains(criteria)) && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "Abb")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                join s in db.Part_Supplier on p.Part_Type_ID equals s.Part_Type_ID
                                orderby p.Name
                                where p.Abbreviation.Contains(criteria) && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
                    ...
                    if (category == "Dimension")
                    {
                        result = JObject.FromObject(new
                        {
                            part_types =
                                ...
                                join s in db.Part_Supplier on p.Part_Type_ID equals s.Part_Type_ID
                                orderby p.Name
                                where p.Dimension.Contains(criteria) && s.Supplier_ID == id
                                select new
                                {
                                    Part_Type_ID = p.Part_Type_ID,
                                    Abbreviation = p.Abbreviation,
            ...
            {
                ExceptionLog.LogException(e, "SearchPartTypeController PUT");
                return "false|An error has occured searching for Part Types on the system.";
            }
        }
